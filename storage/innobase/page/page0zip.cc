...
{
	DBUG_ENTER("page_zip_empty_size");
	lint	size = zip_size
	...
	DBUG_RETURN(size > 0 ? (ulint) size : 0);
}
...
{
	DBUG_ENTER("page_zip_is_too_big");
...
{
	DBUG_ENTER("page_zip_dir_elems");
	/* Exclude the page infimum and supremum from the record count. */
	DBUG_RETURN(page_dir_get_n_heap(page_zip->data) - PAGE_HEAP_NO_USER_LOW);
}
...
/*==============*/
	const page_zip_des_t*	page_zip)	/*!< in: compressed page */
{
	DBUG_ENTER("page_zip_dir_size");
	DBUG_RETURN(PAGE_ZIP_DIR_SLOT_SIZE * page_zip_dir_elems(page_zip));
}
...
{
	DBUG_ENTER("page_zip_dir_start_offs");
	ut_ad(n_dense * PAGE_ZIP_DIR_SLOT_SIZE < page_zip_get_size(page_zip));

	DBUG_RETURN(page_zip_get_size(page_zip) - n_dense * PAGE_ZIP_DIR_SLOT_SIZE);
}
...
/*===================*/
	const page_zip_des_t*	page_zip)	/*!< in: compressed page */
{
	DBUG_ENTER("page_zip_dir_user_size");
...
{
	DBUG_ENTER("page_zip_dir_find_low");
	ut_ad(slot <= end);
...
/*==============*/
	...
	ulint		offset)			/*!< in: offset of user record */
{
	DBUG_ENTER("page_zip_dir_find");
	byte*	end	= page_zip->data + page_zip_get_size(page_zip);

	ut_ad(page_zip_simple_validate(page_zip));

	DBUG_RETURN(page_zip_dir_find_low(end - page_zip_dir_user_size(page_zip),
					 end,
					 offset));
}
...
/*===================*/
	...
	ulint		offset)			/*!< in: offset of user record */
{
	DBUG_ENTER("page_zip_dir_find_free");
	byte*	end	= page_zip->data + page_zip_get_size(page_zip);

	ut_ad(page_zip_simple_validate(page_zip));

	DBUG_RETURN(page_zip_dir_find_low(end - page_zip_dir_size(page_zip),
...
/*=============*/
	...
	ulint			slot)		/*!< in: slot
						(0=first user record) */
{
	DBUG_ENTER("page_zip_dir_get");
	ut_ad(page_zip_simple_validate(page_zip));
	ut_ad(slot < page_zip_dir_size(page_zip) / PAGE_ZIP_DIR_SLOT_SIZE);
	DBUG_RETURN(mach_read_from_2(page_zip->data + page_zip_get_size(page_zip)
...
{
	DBUG_ENTER("page_zip_compress_write_log");
	byte*	log_ptr;
	ulint	trailer_size;
...
{
	DBUG_ENTER("page_zip_get_n_prev_extern");
...
{
	DBUG_ENTER("page_zip_fixed_field_encode");
	ut_ad(val >= 2);
...
{
	DBUG_ENTER("page_zip_fields_encode");
	...
	DBUG_RETURN((ulint) (buf - buf_start));
}
...
/*================*/
	...
	const rec_t**	recs)	/*!< in: pointer to an array of 0, or NULL;
				...
				address (and heap_no) */
{
	DBUG_ENTER("page_zip_dir_encode");
	const byte*	rec;
	ulint		status;
	ulint		min_mark;
	ulint		heap_no;
...
{
	DBUG_ENTER("page_zip_compress_deflate");
	int	status;
...
{
	DBUG_ENTER("page_zip_compress_node_ptrs");
	int	err	= Z_OK;
	ulint*	offsets = NULL;
...
/*==================*/
	...
	ulint		n_dense)	/*!< in: size of recs[] */
{
	DBUG_ENTER("page_zip_compress_sec");
	int		err	= Z_OK;
...
/*========================*/
	...
	ulint*		n_blobs)	/*!< in/out: number of
					externally stored columns */
{
	DBUG_ENTER("page_zip_compress_clust_ext");
	int	err;
	ulint	i;
...
/*====================*/
	...
	mem_heap_t*	heap)		/*!< in: temporary memory heap */
{
	DBUG_ENTER("page_zip_compress_clust");
	int	err		= Z_OK;
	ulint*	offsets		= NULL;
...
{
  DBUG_ENTER("page_zip_fields_free");
  if (index) {
...
{
  DBUG_ENTER("page_zip_fields_decode");
  const byte* b;
  ulint n;
  ulint i;
  ulint val;
...
/*================*/
	...
	ulint			n_dense)/*!< in: number of user records, and
					size of recs[] */
{
	DBUG_ENTER("page_zip_dir_decode");
	ulint	i;
	ulint	n_recs;
	byte*	slot;
...
{
	DBUG_ENTER("page_zip_set_extra_bytes");
	ulint	n;
	ulint	i;
	ulint	n_owned = 1;
	ulint	offs;
...
{
	DBUG_ENTER("page_zip_apply_log_ext");
	ulint	i;
	ulint	len;
	byte*	next_out = rec;
...
{
	DBUG_ENTER("page_zip_decompress_heap_no");
...
/*==========================*/
	...
	mem_heap_t*	heap)		/*!< in: temporary memory heap */
{
	DBUG_ENTER("page_zip_decompress_node_ptrs");
...
/*====================*/
	...
	ulint*		offsets)	/*!< in/out: temporary offsets */
{
	DBUG_ENTER("page_zip_decompress_sec");
...
/*==========================*/
	...
	ulint		trx_id_col)	/*!< in: position of of DB_TRX_ID */
{
	DBUG_ENTER("page_zip_decompress_clust_ext"); 
	ulint	i;
...
/*======================*/
	...
	mem_heap_t*	heap)		/*!< in: temporary memory heap */
{
	DBUG_ENTER("page_zip_decompress_clust");
	int		err;
	ulint		slot;
	ulint		heap_status	= REC_STATUS_ORDINARY
		| PAGE_HEAP_NO_USER_LOW << REC_HEAP_NO_SHIFT;
...
/*====================*/
	...
	ibool		all)	/*!< in: TRUE=decompress the whole page;
				...
				after page creation */
{
	DBUG_ENTER("page_zip_decompress_low");
	z_stream	d_stream;
...
/*================*/
	...
	ibool		all)	/*!< in: TRUE=decompress the whole page;
				...
				after page creation */
{
	DBUG_ENTER("page_zip_decompress");
#ifndef UNIV_HOTBACKUP
...
{
	DBUG_ENTER("page_zip_hexdump_func");
...
{
	DBUG_ENTER("page_zip_validate_low");
...
/*==============*/
	...
	const dict_index_t*	index)	/*!< in: index of the page, if known */
{
	DBUG_ENTER("page_zip_validate");
	DBUG_RETURN(page_zip_validate_low(page_zip, page, index,
					 recv_recovery_is_on()));
}
#endif /* UNIV_ZIP_DEBUG */
...
{
	DBUG_ENTER("page_zip_header_cmp");
...
{
	DBUG_ENTER("page_zip_write_rec_ext");
...
/*==========================*/
	...
	page_zip_des_t*	page_zip)/*!< in/out: compressed page */
{
	DBUG_ENTER("page_zip_parse_write_blob_ptr");
	ulint	offset;
	ulint	z_offset;
...
corrupt:
	...
	DBUG_RETURN(ptr + (2 + 2 + BTR_EXTERN_FIELD_REF_SIZE));
}
...
{
	DBUG_ENTER("page_zip_write_blob_ptr");
	const byte*	field;
	byte*		externs;
	const page_t*	page	= page_align(rec);
	ulint		blob_no;
...
/*==========================*/
	...
	page_zip_des_t*	page_zip)/*!< in/out: compressed page */
{
	DBUG_ENTER("page_zip_parse_write_node_ptr");
	ulint	offset;
	ulint	z_offset;
...
corrupt:
	...
	DBUG_RETURN(ptr + (2 + 2 + REC_NODE_PTR_SIZE));
}
...
{
	DBUG_ENTER("page_zip_write_node_ptr");
	byte*	field;
	byte*	storage;
#ifdef UNIV_DEBUG
...
{
	DBUG_ENTER("page_zip_write_trx_id_and_roll_ptr");
	byte*	field;
	byte*	storage;
#ifdef UNIV_DEBUG
	page_t*	page	= page_align(rec);
...
{
	DBUG_ENTER("page_zip_rec_set_deleted");
...
{
	DBUG_ENTER("page_zip_rec_set_owned");
	byte*	slot = page_zip_dir_find(page_zip, page_offset(rec));
	ut_a(slot);
	UNIV_MEM_ASSERT_RW(page_zip->data, page_zip_get_size(page_zip));
	if (flag) {
...
{
	DBUG_ENTER("page_zip_dir_insert");
	ulint	n_dense;
	byte*	slot_rec;
	byte*	slot_free;
...
/*================*/
	...
	const byte*		free)		/*!< in: previous start of
						the free list */
{
	DBUG_ENTER("page_zip_dir_delete");
	byte*	slot_rec;
	byte*	slot_free;
	ulint	n_ext;
	page_t*	page	= page_align(rec);
...
/*==================*/
	...
	ulint		is_clustered)	/*!< in: nonzero for clustered index,
					zero for others */
{
	DBUG_ENTER("page_zip_dir_add_slot");
	ulint	n_dense;
	byte*	dir;
	byte*	stored;
...
/*========================*/
	...
	page_zip_des_t*	page_zip)/*!< in/out: compressed page */
{
	DBUG_ENTER("page_zip_parse_write_header");
	ulint	offset;
	ulint	len;
...
/*======================*/
	...
	mtr_t*		mtr)	/*!< in: mini-transaction */
{
	DBUG_ENTER("page_zip_write_header_log");
...
/*================*/
	...
	mtr_t*		mtr)	/*!< in: mini-transaction */
{
	DBUG_ENTER("page_zip_reorganize");
#ifndef UNIV_HOTBACKUP
...
{
	DBUG_ENTER("page_zip_parse_compress");
	ulint	size;
	ulint	trailer_size;
...
corrupt:
	...
	DBUG_RETURN(ptr + 8 + size + trailer_size);
}
...
{
	DBUG_ENTER("page_zip_calc_checksum");
	uint32_t	adler;
	...
	case SRV_CHECKSUM_ALGORITHM_STRICT_NONE:
		DBUG_RETURN(BUF_NO_CHECKSUM_MAGIC);
...
{
	DBUG_ENTER("page_zip_verify_checksum");
